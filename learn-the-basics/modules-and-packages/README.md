...
- However, any namespace cannot have two objects with the exact same name,
  <br />so the `import` command may replace an existing object in the namespace.
...
- A bit risky as changes in the module (`draw`) might affect the module (`game`) which imports it.
...
When found the function (e.g. `urlopen`) in the module (e.g. `urllib.request`) we want to use,
