...
We also must not forget to add the **init**.py file inside the foo directory.
...
The **init**.py file can also decide which modules the package exports as the API,
while keeping other modules internal, by overriding the **all** variable, like so:
